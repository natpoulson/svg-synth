...
class Prompt {
        ...
        const palette = [];
            for (const colour of Object.keys(Prompt.colour)) {
                ...
                Prompt.colour[colour].forEach(a => {
                    // Push objects with the name and its formatted equivalent (which we'll use in the SVG markup)
    ...
    static formatColour(colourString) {
        // Regex strip spaces and lowercase the colour name to format it in SVG compatible syntax
    ...
    static questions = {
        simple: [
            ...
            },
            {
                ...
                type: 'input',
                validate: (input) => /^.{0,3}$/.test(input) // Added layer of security to ensure 3 character answers must be provided
            }
        ]
